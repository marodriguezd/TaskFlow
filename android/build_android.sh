...
missing=()
for tool in git zip unzip javac make autoconf automake autoreconf aclocal libtool libtoolize pkg-config cmake find readlink mktemp; do
...
# Esto evita errores tipo AC_PROG_LIBTOOL / AC_PROG_LD undefined durante libffi/autoreconf.
...
IFS=':' read -r -a ACLOCAL_DIRS <<< "$ACLOCAL_PATH"
INCLUDE_ARGS=()
for dir in "${ACLOCAL_DIRS[@]}"; do
  [[ -d "$dir" ]] && INCLUDE_ARGS+=("-I" "$dir")
done
if (( ${#INCLUDE_ARGS[@]} == 0 )); then
  echo "ERROR: ACLOCAL_PATH no contiene directorios válidos: $ACLOCAL_PATH"
  exit 1
fi

# Sanity-check: verificar macros críticas en rutas detectadas.
if ! find "${ACLOCAL_DIRS[@]}" -maxdepth 1 -type f -name '*.m4' -print 2>/dev/null | xargs -r grep -E 'AC_PROG_LIBTOOL|LT_INIT|AC_PROG_LD' -q; then
...
# Forzamos includes también vía wrappers para que python-for-android/autoreconf los vea siempre.
WRAP_DIR="$(mktemp -d)"
trap 'rm -rf "$WRAP_DIR"' EXIT

REAL_ACLOCAL="$(command -v aclocal)"
REAL_AUTORECONF="$(command -v autoreconf)"

cat > "$WRAP_DIR/aclocal" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "$REAL_ACLOCAL" "\$@" ${INCLUDE_ARGS[*]}
EOF
chmod +x "$WRAP_DIR/aclocal"

cat > "$WRAP_DIR/autoreconf" <<EOF
#!/usr/bin/env bash
set -euo pipefail
export ACLOCAL="$WRAP_DIR/aclocal"
exec "$REAL_AUTORECONF" ${INCLUDE_ARGS[*]} "\$@"
EOF
chmod +x "$WRAP_DIR/autoreconf"
...
echo "[build_android] Autoreconf wrapper activo: $WRAP_DIR/autoreconf"
